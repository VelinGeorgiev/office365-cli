...
describe(commands.PROPERTYBAG_REMOVE, () => {
  ...
  it('should property bag property be removed successfully (verbose) without prompting with confirmation argument', (done) => {
    stubAllPostRequests();
  ...
  it('should prompt before removing property bag property when confirmation argument not passed', (done) => {
    auth.site = new Site();
    auth.site.connected = true;
    auth.site.url = 'https://contoso-admin.sharepoint.com';
    cmdInstance.action = command.action();
  ...
  it('should abort property bag property remove when prompt not confirmed', (done) => {
  ...
  it('should remove property bag property when prompt confirmed', (done) => {
    const postCallsSpy: sinon.SinonStub = stubAllPostRequests();
